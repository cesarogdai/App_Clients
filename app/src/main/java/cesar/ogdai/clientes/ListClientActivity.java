...
public class ListClientActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(R.string.all_clients);

        conn = new SQLHelper(getApplicationContext(), "bd_clientes", null,1);

        allClientsListView = (ListView) findViewById(R.id.listViewAllClients);

        consultClients();

        ArrayAdapter adapter = new ArrayAdapter(this, android.R.layout.simple_list_item_1, listInfo);
        allClientsListView.setAdapter(adapter);
        allClientsListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                info += "date: "+listUser.get(position).getAction()+"\n";

                User user = listUser.get(position);

                Intent intent = new Intent(ListClientActivity.this, ClientDetails.class);
    ...
    private void consultClients(){
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM " + Utilidades.Table_Clients,null );
